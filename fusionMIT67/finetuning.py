...
def main():
    ...
    summary_dir='/home/lzy/summary/generateDepth/'+'finetuning_2e-4_2fcor_fix2_'+str(run_id)
    ...
    optimizer= torch.optim.SGD(policies, cfg.LR,
                            ...
                            weight_decay=cfg.WEIGHT_DECAY)
    schedulers = get_scheduler(optimizer, cfg, cfg.LR_POLICY)
    for epoch in range(cfg.START_EPOCH,cfg.EPOCHS+1):

        printlr(optimizer,writer,epoch)
        ...
        train(train_loader,schedulers,model,criterion,generate_model,optimizer,epoch,writer)
        mean_acc=validate(val_loader,model,criterion,generate_model,epoch,writer)
    ...
    writer.close()
def train(train_loader,schedulers,model,criterion,generate_model,optimizer,epoch,writer):
    losses = AverageMeter()
    losses = AverageMeter()
    model.train()
    ...
    for i, (input,target) in enumerate(train_loader):
        ...
        global_all_step+=1
        update_learning_rate(schedulers,optimizer,writer,epoch=global_all_step)
        out_keys=[]
