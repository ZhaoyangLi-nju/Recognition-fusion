...
def define_TrecgNet(cfg, use_noise=None, upsample=None, device=None,generate=False):
    if upsample is None:
        upsample = not cfg.NO_UPSAMPLE

    if 'resnet' in cfg.ARCH:
        if generate:
            model = TRecgNet_generatedepth(cfg, encoder=cfg.ARCH, upsample=upsample,
...
class TRecgNet_Upsample_Resiual(nn.Module):
    ...
    def build_upsample_layers(self, dims):
        ...
        self.deconv1 = Upsample_Interpolate(dims[6], dims[5], kernel_size=1, padding=0, norm=norm)
        self.deconv2 = Upsample_Interpolate(dims[5], dims[4], kernel_size=1, padding=0, norm=norm)
        self.deconv3 = Upsample_Interpolate(dims[4], dims[3], kernel_size=1, padding=0, norm=norm)
        self.deconv4 = Upsample_Interpolate(dims[3], dims[3], kernel_size=3, padding=1, norm=norm)

        skip3_channel = dims[5]
        skip2_channel = dims[4]
        skip1_channel = dims[3]
    ...
    def forward(self, source=None, target=None, label=None, out_keys=None, phase='train', content_layers=None, return_losses=True):
        ...
        return loss_cls
class TRecgNet_generatedepth(nn.Module):

    def __init__(self, cfg, encoder='resnet18', upsample=True,
                 use_noise=False, device=None):
        super(TRecgNet_generatedepth, self).__init__()
